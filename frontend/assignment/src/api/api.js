...
const getProducts = async () =>
  tryRequest(async () => (await axios.get(`${BASE}/products`)).data, () => mockProducts.slice());

const getCart = async () =>
  tryRequest(async () => (await axios.get(`${BASE}/cart`)).data, () => JSON.parse(JSON.stringify(inMemoryCart)));

const addToCart = async (productId, qty = 1) =>
  tryRequest(
    async () => (await axios.post(`${BASE}/cart/add`, { productId, qty })).data,
    () => {
      const prod = mockProducts.find((p) => p._id === productId) || mockProducts[0];
      const idx = inMemoryCart.items.findIndex((i) => i.productId === productId);
      if (idx !== -1) {
        inMemoryCart.items[idx].qty += Number(qty);
      } else {
        inMemoryCart.items.push({ productId: prod._id, name: prod.name, price: prod.price, qty: Number(qty) });
      }
      recalcCart();
...
const removeFromCart = async (productId) =>
  tryRequest(
    ...
    () => {
      inMemoryCart.items = inMemoryCart.items.filter((i) => i.productId !== productId);
      recalcCart();
      return JSON.parse(JSON.stringify(inMemoryCart));
    }
  );
...
const updateCart = async (productId, qty) =>
  tryRequest(
    async () => (await axios.put(`${BASE}/cart/update`, { productId, qty })).data,
    () => {
      const idx = inMemoryCart.items.findIndex((i) => i.productId === productId);
      if (idx === -1) {
        if (qty > 0) {
...
const checkout = async (cartItems = [], user = {}) =>
  tryRequest(
    async () => (await axios.post(`${BASE}/checkout`, { cartItems, user })).data,
    () => {
      // create fake receipt and clear in-memory cart
      const items = cartItems.length > 0 ? cartItems : inMemoryCart.items.map((i) => ({ productId: i.productId, qty: i.qty, price: i.price, name: i.name }));
      const receiptItems = items.map((i) => ({ productId: i.productId, name: i.name, qty: i.qty, price: i.price, lineTotal: i.qty * i.price }));
      const total = receiptItems.reduce((s, it) => s + it.lineTotal, 0);
      const receipt = { id: `rcpt_mock_${Date.now()}`, items: receiptItems, total, timestamp: new Date().toISOString() };
      inMemoryCart = { items: [], total: 0 };
      return { receipt };
    }
  );

export default { getProducts, getCart, addToCart, removeFromCart, updateCart, checkout };
